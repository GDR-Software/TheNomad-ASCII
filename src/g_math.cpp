...
static constexpr int32_t FIXED_POINT_SCALE = 1000;

nomadfixed_t to_fixed(nomadfloat_t x) {
    return x * (1 << 16);
}

nomadfixed_t mult_fixed(nomadfixed_t x, nomadfixed_t y) {
	return ((int64_t) x) * y / FIXED_POINT_SCALE;
}
nomadfixed_t div_fixed(nomadfixed_t x, nomadfixed_t y) {
	return ((int64_t) x) * FIXED_POINT_SCALE / y;
}

nomadfloat_t to_float(nomadfixed_t x) {
	return x / double(1 << 10);
}

inline nomadfloat_t dotProduct()
{
